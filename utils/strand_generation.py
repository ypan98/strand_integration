...
# # difference wrt original implementation: there is a mask to avoid revisiting points and stuck in infinite loop in the case of a single neighbor
# def accumulate_strand_points(kdtree, mask, points: np.ndarray, directions: np.ndarray, initial_point: np.ndarray, initial_direction: np.ndarray, step_size: float, 
...
#     nn_indices = nn_indices[mask[nn_indices]] if len(nn_indices) > 0 else nn_indices
...
# # Implementation following forward euler method from Strand-accurate Multi-view Hair Capture paper
...
#         strand = []
#         remaining_points = points[remaining_points_mask]
#         remaining_directions = directions[remaining_points_mask]
#         mask = np.ones(remaining_points.shape[0], dtype=bool)
#         kdtree = cKDTree(remaining_points)
#         # 1. Pick random seed from remaining points
#         seed_idx = np.random.randint(remaining_points.shape[0])
#         seed_point = remaining_points[seed_idx]
#         seed_direction = remaining_directions[seed_idx]
#         strand.append(seed_point)
#         mask[seed_idx] = False
#         # 2. Accumulate strand points from forward and backward directions
#         forward_points = accumulate_strand_points(kdtree, mask, remaining_points, remaining_directions, seed_point, seed_direction, step_size, dist_th, angle_th)
#         backward_points = accumulate_strand_points(kdtree, mask, remaining_points, remaining_directions, seed_point, -seed_direction, step_size, dist_th, angle_th)
#         strand = forward_points + backward_points + strand
#         strand = np.array(strand)
#         strands.append(strand)        
#         # 3. Remove points that are nearby current strand
#         curr_strand_mask = np.ones(remaining_points.shape[0], dtype=bool)
...
#         curr_strand_mask[arr_indices] = False
#         remaining_points_mask[remaining_points_mask] = curr_strand_mask
#         # updae progress bar
