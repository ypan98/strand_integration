...
if __name__ == '__main__':
    ...
    for i in range(N):
        ...
        view_output_folder = output_folder / view_id
        view_output_folder.mkdir(exist_ok=True, parents=True)
        # image
        image_file = image_folder / f"image_{id}.png"
        ...
        # mask
        mask_file = mask_folder / f"image_{id}.png"
        mask = cv2.imread(mask_file)
        cv2.imwrite(str(view_output_folder / "mask.png"), mask)
        # orientation and confidence
        if not args.skip_orientation:
            orientation_file = orientation_folder / f"image_{id}_orientation.png"
            confidence_file = orientation_folder / f"image_{id}_confidence.png"
        ...
        # K, R, t
        R, t = colmap_camera_extrinsic_to_Rt(cam_extrinsics[id], scale=args.scale)
        K = colmap_camera_intrinsic_to_K(cam_intrinsics[id])
